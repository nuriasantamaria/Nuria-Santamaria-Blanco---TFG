...
_B_MAX    = 3e-5    # [T]
_BDOT_MAX = 4.5e-6  # [T/s]

_OBS_DIM = 6   # el wrapper apila a_prev; el entorno expone solo B_curr y Ḃ


class UPMSat2EnvPPO(gym.Env):
    ...
    def __init__(self,
        ...
        self._omega_detumble_thresh = omega_detumble_thresh   # ~0.143 °/s por defecto
    ...
    def _validate_inertia(self,
        ...
        """Valida tensor de inercia 3×3 (simétrico y definido positivo)."""
    ...
    def _get_observation(self) -> np.ndarray:
        """obs = [B_b(t_k)/B_MAX, Ḃ_b(t_k)/BDOT_MAX] (6D). Ḃ se expone
        explícitamente para no depender de que el wrapper la infiera
        entre frames; con history_length=1 el vector final queda en 9D."""
        b_norm    = np.clip(self._B_b_meas_cache / _B_MAX,    -1.0, 1.0)
        bdot_norm = np.clip(self._B_dot_cache    / _BDOT_MAX, -1.0, 1.0)
        return np.concatenate([b_norm, bdot_norm]).astype(np.float32)

    def _compute_reward(self, m: np.ndarray) -> float:
        """
        Recompensa teacher-student (idéntica al entorno 12D):
        r = w_det·r_det + r_energy + r_smooth + r_bonus + r_gaussian.
        r_smooth usa _last_action/_prev_action, rastreadas internamente
        aunque a_prev no esté en la observación.
        """
    ...
    def _check_done(self) -> tuple[bool, bool]:
        """terminated siempre False (sin estado terminal natural); truncated si se
        supera max_steps o diverge (‖ω‖ > omega_max)."""
    ...
    def _get_info(self) -> dict:
        """Métricas de diagnóstico, idénticas al entorno 12D para que main.py funcione sin cambios."""
        omega      = self._state[4:]
